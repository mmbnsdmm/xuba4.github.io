<template>
    <view class="user-profile-tags">
        <ol class="breadcrumb">
            ...
            <li class="active">用户圈子</li>
            <text class="text-blue pull-right" @tap="toCreateCircle">创建圈子</text>
        </ol>
        <div class="row">
            <div class="col-xs-12">
                <WLoadMore ref="WODROW_LOAD_MORE_PROFILE_TAG_LIST" @provider="provider" :pageSize="page_size" color="#66ccff">
                        ...
                        <view class="solid-top" v-for="(item, index) in items" :key="index">
                            <u-card padding="10" margin="15rpx" :show-head="false" :border="false" :head-border-bottom="false" :foot-border-top="false" title-size="15rpx">
                                ...
                                <view class="" slot="foot">
                                    ...
                                    <u-icon v-if="item.isYouJoin" name="close-circle" size="34" color="" label="退出" class="pull-right text-danger" @tap="toQuit(index)"></u-icon>
                                    <u-icon v-if="!item.isYouJoin" name="plus-circle" size="34" color="" label="加入" class="pull-right text-danger" @tap="toJoin(index)"></u-icon>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.$refs.WODROW_LOAD_MORE_PROFILE_TAG_LIST.reLoadData()
        },
        methods: {
            provider(pd){
                let _this = this;
                setTimeout(function () {
                    ...
                    _this.$refs.WODROW_LOAD_MORE_PROFILE_TAG_LIST.pushData(res);
                }, 1000);
            },
            getData(pd) {
                let _this = this;
            ...
            toJoin(index){
                ...
                Dialog.confirm({
                    ...
                    let formParams = {
                        id: _this.$refs.WODROW_LOAD_MORE_PROFILE_TAG_LIST.getItem(index).id
                    };
                    _this.$auth.post("/tag/default/join", formParams, true, function (res) {
                        _this.$refs.WODROW_LOAD_MORE_PROFILE_TAG_LIST.updateItem(index, res.tag);
                    }, function (msg) {
                        Toast(msg);
                    });
                }).catch(() => {});
        ...
        onPullDownRefresh() {
            this.$refs.WODROW_LOAD_MORE_PROFILE_TAG_LIST.pullDownRefresh();
        },
        onReachBottom() {
            this.$refs.WODROW_LOAD_MORE_PROFILE_TAG_LIST.reachBottom()
        }
    }
</script>
